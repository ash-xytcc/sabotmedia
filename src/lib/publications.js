...
export function createPublication(overrides = {}) {
  ...
  return {
    id: makeId('pub'),
    type: overrides.type || 'Zine',
    publicationType: overrides.publicationType || overrides.type || 'Zine',
    title,
    slug: slugifyPublication(title),
    status: 'draft',
    visibility: 'draft',
    subtitle: '',
    description: '',
    ...
    pieceSlugs: [],
    printlabProjectId: '',
    printlabProjectUrl: '/wp-admin/printlab',
    archiveEntrySlug: '',
    downloadPageSlug: '',
    downloadAssets: [],
    versions: [],
...
export async function loadPublicationsAsync(params = {}) {
  ...
  try {
    ...
    if (res.ok && data?.ok && Array.isArray(data.items) && data.mode !== 'scaffold') {
      const normalized = data.items.map(normalizePublication)
      savePublications(normalized)
      return normalized
    }
  } catch {
    // Static/dev fallback.
  ...
  if (typeof window !== 'undefined') {
    try {
      ...
      if (res.ok && data?.ok && data.item && data.mode !== 'scaffold') {
        return savePublication(data.item)
      }
    } catch {
      // Static/dev fallback.
    }
  }
  return savePublication(normalized)
}
...
export function generatePublicationFromPieces(publication, pieces = []) {
  ...
  pages.push(createPage('cover', pages.length, {
    title: 'Cover',
    blocks: [
  ...
  if (source.frontMatter) {
    pages.push(createPage('inside', pages.length, {
      title: 'Front Matter',
      blocks: [createTextBlock({ x: 80, y: 90, width: 620, height: 720, text: source.frontMatter, fontSize: 22 })],
    }))
  }

  const toc = selectedPieces.map((piece, index) => ({
  ...
  pages.push(createPage('inside', pages.length, {
    ...
    blocks: [createTextBlock({
      ...
      width: 620,
      height: 720,
      text: ['Table of Contents', ...toc.map((item, index) => `${index + 1}. ${item.title}`)].join('\n\n'),
      fontSize: 24,
    })],
  }))
  ...
  pages.push(createPage('inside', pages.length, {
    ...
    blocks: [createTextBlock({
      ...
      width: 620,
      height: 720,
      text: ['Credits', source.credits, '', 'Colophon', source.colophon].filter((line) => line != null).join('\n'),
      fontSize: 22,
    })],
  }))

  pages.push(createPage('back-cover', pages.length, {
    title: 'Back Cover',
    blocks: [createTextBlock({ x: 80, y: 120, width: 620, height: 540, text: source.backCover || source.description || 'Sabot Media', fontSize: 28 })],
  }))

  const version = {
    id: makeId('version'),
    label: `Generated ${new Date(generatedAt).toLocaleString()}`,
    createdAt: generatedAt,
    summary: `Generated ${pages.length} pages from ${selectedPieces.length} pieces.`,
    pageCount: pages.length,
  }

  return updatePublicationPages({
    ...source,
    status: source.visibility === 'public' ? 'published' : source.status,
    tableOfContents: toc,
    pages,
    generatedAt,
    archiveEntrySlug: source.archiveEntrySlug || source.slug,
    downloadPageSlug: source.downloadPageSlug || source.slug,
    versions: [version, ...(source.versions || [])].slice(0, 30),
  }, pages)
}
...
function normalizeVisibilityFromStatus(status) {
...
function normalizeDownloadAssets(value) {
  ...
  return arr.map((asset) => ({
    id: String(asset?.id || makeId('download')),
    title: String(asset?.title || ''),
    url: String(asset?.url || ''),
    type: String(asset?.type || ''),
    visibility: String(asset?.visibility || 'public'),
  })).filter((asset) => asset.title || asset.url)
}

function normalizeVersions(value) {
  const arr = Array.isArray(value) ? value : []
  return arr.map((version) => ({
    id: String(version?.id || makeId('version')),
