export function selectHubCoverage(items = [], limit = 8) {
  ...
  const editorial = sorted.filter((item) => item.automated !== true)
  const discovered = sorted.filter((item) => item.automated === true)
  return [...editorial, ...discovered.filter((item) => !editorial.some((chosen) => sameCoverage(chosen, item)))].slice(0, limit)
}
