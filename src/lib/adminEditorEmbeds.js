...
export function buildIframeEmbed(value = '', title = 'Embedded content') {
  ...
  if (!src) return ''
  return `<figure class="sabot-embed sabot-embed--iframe"><iframe src="${escapeAttribute(src)}" title="${escapeAttribute(title || 'Embedded content')}" loading="lazy" referrerpolicy="no-referrer" allowfullscreen></iframe></figure><p><br /></p>`
}
...
export function buildMediaEmbed(media = {}) {
  ...
  if (type === 'image' || type === 'svg' || mime.startsWith('image/')) {
    const image = `<img src="${escapedUrl}" alt="${escapeAttribute(alt)}" />`
    return caption
      ? `<figure>${image}<figcaption>${escapedCaption}</figcaption></figure><p><br /></p>`
      : `${image}<p><br /></p>`
  }

  if (type === 'audio' || mime.startsWith('audio/') || /\.(mp3|m4a|aac|ogg|oga|wav|webm)$/.test(lowerUrl)) {
    return `<figure class="sabot-embed sabot-embed--audio"><audio controls preload="metadata" src="${escapedUrl}"></audio>${caption ? `<figcaption>${escapedCaption}</figcaption>` : ''}</figure><p><br /></p>`
  }

  if (mime === 'application/pdf' || lowerUrl.endsWith('.pdf')) {
    const label = escapedCaption || escapedTitle || 'Open PDF'
    return `<figure class="sabot-embed sabot-embed--pdf"><iframe src="${escapedUrl}" title="${escapeAttribute(title || 'PDF document')}" loading="lazy"></iframe><figcaption><a href="${escapedUrl}" target="_blank" rel="noopener noreferrer">${label}</a></figcaption></figure><p><br /></p>`
  }
