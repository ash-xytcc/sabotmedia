...
export function CampaignPage() {
  ...
  useEffect(() => {
    ...
    async function refreshDashboard() {
      try {
        const loadedCampaign = await loadCampaign(campaignSlug, { includeDrafts: previewDraft })
        if (cancelled) return
        setCampaign(loadedCampaign)
        setError('')
        hasLoaded = true
        loadPublishedNativePieces()
          .then((loadedPieces) => { if (!cancelled) setPieces(Array.isArray(loadedPieces) ? loadedPieces : []) })
          .catch(() => {})
      } catch (err) {
  ...
  useEffect(() => {
    if (!campaign || !isAiCampaign || previewDraft) return undefined
    let cancelled = false
    async function refreshLiveSources() {
      const [socialResult, intelligenceResult] = await Promise.allSettled([
        fetch('/api/campaign-social?slug=' + encodeURIComponent(campaignSlug), { headers: { accept: 'application/json' } }).then(async (response) => {
          const data = await response.json().catch(() => ({}))
          if (!response.ok) throw new Error(data?.error || ('campaign live source failed: ' + response.status))
          return data
        }),
        fetch('/api/campaign-intelligence', { headers: { accept: 'application/json' } }).then(async (response) => {
          const data = await response.json().catch(() => ({}))
          if (!response.ok) throw new Error(data?.error || ('campaign live source failed: ' + response.status))
          return data
        }),
      ...
      if (cancelled) return
      setCampaign((current) => {
        if (!current || current.slug !== campaignSlug) return current
        ...
        return {
          ...current,
          ...(socialData ? {
            social: mergeByIdentity(socialData.items || [], current.social || []),
            socialSources: socialData.sources || [],
            socialErrors: socialData.errors || [],
            socialCheckedAt: socialData.checkedAt || '',
          } : {}),
          ...(intelligenceData ? {
            updates: mergeByIdentity(intelligenceData.updates || [], current.updates || []),
            coverage: mergeByIdentity(intelligenceData.coverage || [], current.coverage || []),
            intelligenceSources: intelligenceData.sources || [],
            intelligenceErrors: intelligenceData.errors || [],
            intelligenceCheckedAt: intelligenceData.checkedAt || '',
          } : {}),
        }
      })
    }
    refreshLiveSources()
    const timer = window.setInterval(refreshLiveSources, 300000)
    return () => { cancelled = true; window.clearInterval(timer) }
  }, [campaign?.id, campaignSlug, isAiCampaign, previewDraft])
...
function mergeByIdentity(primary = [], fallback = []) {
  const seen = new Set()
  return [...primary, ...fallback].filter((item) => {
    const key = String(item?.id || item?.url || item?.href || ((item?.title || '') + '|' + (item?.date || '')))
