...
function buildPlaintext({ name, replyEmail, subject, message }) {
  return [
    'Sabot Media secure contact form',
...
export function SecureContactForm() {
  ...
  useEffect(() => {
    ...
    async function loadPublicKey() {
      try {
        ...
        const identityMatches = key.getUserIDs().some((identity) => identity.includes('info@sabot.media'))

        if (fingerprint !== EXPECTED_FINGERPRINT || !identityMatches) {
          throw new Error('The public key did not match the pinned Sabot Media identity.')
        }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const mailto = new URL('mailto:info@sabot.media')
  ...
  return (
    <section className="secure-contact" aria-labelledby="secure-contact-title">
      ...
      <p className="secure-contact__intro">
        This form encrypts your message in this browser for <strong>info@sabot.media</strong>, then opens your email app. Nothing is uploaded to Sabot Media by the form itself.
      </p>
