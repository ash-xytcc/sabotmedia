...
export function loadMediaLibraryItems(nativeItems = null) {
  ...
  return dedupeMedia([...nativeMedia, ...importedMedia])
}
...
export function MediaPickerModal({ open, onClose, onPick }) {
  ...
  const visible = items.filter((item) => [item.title, item.url].join(' ').toLowerCase().includes(query.toLowerCase()))
    ...
    <div className="wp-media-modal" role="dialog" aria-modal="true" aria-label="Media Picker">
      <div className="wp-media-modal__panel">
        ...
        <div className="wp-media-toolbar">
          ...
          <button type="button" className="button" title="Upload not wired yet">Add New (upload not wired yet)</button>
        </div>
          ...
          <aside className="wp-media-modal__details">
            ...
            {selected ? (
              <>
                ...
                <button type="button" className="button button--primary" onClick={() => onPick(selected)}>Select</button>
              </>
