...
function makeDownload() {
  return {
    id: `download-${Math.random().toString(36).slice(2, 10)}`,
    title: '',
    url: '',
    type: 'PDF',
    visibility: 'public',
...
export function PublicationSystemPage() {
  ...
  useEffect(() => {
    loadPublicationsAsync({ includeDrafts: 1 }).then((loaded) => {
  ...
  function startNew() {
    const fresh = createPublication({ title: 'Untitled Publication' })
  ...
  async function generatePublication() {
    const generated = generatePublicationFromPieces({
      ...draft,
      printlabProjectUrl: `${adminRoutes.printlab}?publication=${encodeURIComponent(draft.id)}`,
    }, allPieces)
    await saveDraft(generated, 'Publication generated.')
  }
  ...
  function togglePiece(piece) {
    ...
    setDraft((current) => {
      ...
      return {
        ...current,
        pieceSlugs: slugs.includes(slug) ? slugs.filter((item) => item !== slug) : [...slugs, slug],
      }
    })
  }
    ...
    <AdminFrame>
      <main className="page wp-admin-screen publication-system-page">
        <div className="wp-screen-header">
          <div>
            <h1>Publications</h1>
            <p className="description">Build books, magazines, zines, readers, pamphlets, poster packs, campaign kits, and booklets above Printlab.</p>
          </div>
          <button className="button button--primary" type="button" onClick={startNew}>Create Publication</button>
        </div>
        <WpAdminNotices />
        ...
        <div className="publication-builder-layout">
          <aside className="wp-meta-box publication-builder-list">
            ...
            {publications.length ? publications.map((publication) => (
              <button
                type="button"
                className={`publication-builder-list__item${publication.id === activeId ? ' is-active' : ''}`}
                key={publication.id}
                onClick={() => editPublication(publication)}
              >
                <strong>{publication.title}</strong>
                <span>{publication.publicationType} / {publication.visibility} / {publication.pages.length} pages</span>
              </button>
            )) : <p className="description">No publications yet.</p>}
          </aside>

          <section className="publication-builder">
            <section className="wp-meta-box">
              <div className="publication-builder__workflow" aria-label="Publication workflow">
            ...
            <section className="wp-meta-box publication-builder__grid">
              <div>
                ...
                <label className="native-content-editor__field">
                  <span>Title</span>
                  <input value={draft.title || ''} onChange={(event) => patchDraft({ title: event.target.value, slug: draft.slug ? draft.slug : slugifyPublication(event.target.value) })} />
                </label>
                <label className="native-content-editor__field">
                  <span>Slug</span>
                  <input value={draft.slug || ''} onChange={(event) => patchDraft({ slug: slugifyPublication(event.target.value) })} />
                </label>
                <label className="native-content-editor__field">
                  <span>Type</span>
                  <select value={draft.publicationType || 'Zine'} onChange={(event) => patchDraft({ publicationType: event.target.value, type: event.target.value })}>
                    {PUBLICATION_TYPES.map((type) => <option value={type} key={type}>{type}</option>)}
                  </select>
                </label>
                <label className="native-content-editor__field">
                  <span>Visibility</span>
                  <select value={draft.visibility || 'draft'} onChange={(event) => patchDraft({ visibility: event.target.value })}>
                    {PUBLICATION_VISIBILITY.map((value) => <option value={value} key={value}>{value}</option>)}
                  </select>
                </label>
                <label className="native-content-editor__field">
                  <span>Issue number</span>
                  <input value={draft.issueNumber || ''} onChange={(event) => patchDraft({ issueNumber: event.target.value })} />
                </label>
                <label className="native-content-editor__field">
                  <span>Edition</span>
                  <input value={draft.edition || ''} onChange={(event) => patchDraft({ edition: event.target.value })} />
                </label>
                <label className="native-content-editor__field">
              ...
              <div>
                ...
                {draft.coverImage ? <img className="publication-builder__cover" src={draft.coverImage} alt="" /> : null}
                <label className="native-content-editor__field">
                ...
                <label className="native-content-editor__field">
                  ...
                  <textarea value={draft.frontMatter || ''} onChange={(event) => patchDraft({ frontMatter: event.target.value })} />
                </label>
                <label className="native-content-editor__field">
                  <span>Credits</span>
                  <textarea value={draft.credits || ''} onChange={(event) => patchDraft({ credits: event.target.value })} />
                </label>
                <label className="native-content-editor__field">
                  <span>Colophon</span>
                  <textarea value={draft.colophon || ''} onChange={(event) => patchDraft({ colophon: event.target.value })} />
                </label>
                <label className="native-content-editor__field">
                  <span>Back cover</span>
                  <textarea value={draft.backCover || ''} onChange={(event) => patchDraft({ backCover: event.target.value })} />
                </label>
              </div>
            </section>
            ...
            <section className="wp-meta-box">
              ...
              <div className="publication-piece-picker">
                <div>
                  <h3>Archive</h3>
                  {filteredPieces.slice(0, 100).map((piece) => {
                    const slug = resolveSlug(piece)
                    const checked = normalizeList(draft.pieceSlugs).includes(slug)
                    return (
                ...
                <div>
                  <h3>Publication Order</h3>
                  {selectedPieces.length ? selectedPieces.map((piece) => {
                    const slug = resolveSlug(piece)
                    return (
                      <div className="publication-selected-piece" key={slug}>
                        <strong>{piece.title || slug}</strong>
                        <span>{piece.contentType || piece.type}</span>
                        <div>
                          <button className="button" type="button" onClick={() => movePiece(slug, -1)}>Up</button>
                          <button className="button" type="button" onClick={() => movePiece(slug, 1)}>Down</button>
                        </div>
                      </div>
                    )
                  }) : <p className="description">Select pieces to arrange the publication.</p>}
                </div>
              </div>
            </section>

            <section className="wp-meta-box">
              <div className="publication-builder__section-header">
                <h2>Downloads and Editions</h2>
                <button className="button" type="button" onClick={() => patchDraft({ downloadAssets: [...(draft.downloadAssets || []), makeDownload()] })}>Add Download</button>
              </div>
              {(draft.downloadAssets || []).map((asset, index) => (
                <div className="publication-download-editor" key={asset.id || index}>
                  <input value={asset.title || ''} onChange={(event) => {
                    const next = [...(draft.downloadAssets || [])]
                    next[index] = { ...asset, title: event.target.value }
                    patchDraft({ downloadAssets: next })
                  }} placeholder="Title" />
                  ...
                  <input value={asset.type || ''} onChange={(event) => {
                    const next = [...(draft.downloadAssets || [])]
                    next[index] = { ...asset, type: event.target.value }
                    patchDraft({ downloadAssets: next })
                  }} placeholder="Type" />
              ...
              <label className="native-content-editor__field">
                <span>Printlab project URL</span>
                <input value={draft.printlabProjectUrl || ''} onChange={(event) => patchDraft({ printlabProjectUrl: event.target.value })} />
              </label>
            </section>

            <section className="wp-meta-box publication-builder__actions">
              <button className="button" type="button" onClick={() => saveDraft()}>Save</button>
              <button className="button button--primary" type="button" onClick={generatePublication}>Generate Publication</button>
            ...
            <section className="wp-meta-box">
              ...
              {(draft.versions || []).length ? draft.versions.map((version) => (
                <article className="publication-version" key={version.id}>
                  <strong>{version.label}</strong>
                  <span>{version.summary}</span>
                  <small>{version.pageCount} pages</small>
                </article>
              )) : <p className="description">Generate the publication to create the first version.</p>}
            </section>
          </section>
        </div>
        ...
        <MediaPickerModal
          ...
          onPick={(item) => {
            patchDraft({ coverImage: item?.url || item?.dataUrl || '', coverAlt: item?.alt || draft.coverAlt || '' })
            setOpenMedia(false)
          }}
        />
      </main>
