  ...
  shell.innerHTML = `
    ...
    <section class="audio-lab-task-grid audio-lab-task-grid--transcript">
      <article class="audio-lab-task-card audio-lab-task-card--wide">
        ...
        <textarea id="audio-lab-transcript-text" class="audio-lab-transcript-editor" placeholder="Transcript text goes here.">${escapeHtml(transcript.text || '')}</textarea>
      ...
      <aside class="audio-lab-task-card">
        <p class="audio-lab-eyebrow">Transcript notes</p>
        <h2>Manual workspace</h2>
        <p>Type transcript text directly or import a .txt, .vtt, or .srt file, then save it with the project.</p>
        <dl class="audio-lab-task-facts">
          <div><dt>Current cues</dt><dd>${transcript.cues?.length || 0}</dd></div>
          <div><dt>Updated</dt><dd>${escapeHtml(transcript.updatedAt || 'never')}</dd></div>
        </dl>
