...
Current reconciled `main`: `0d8306babe8b9c17133db7eec5bbf19128de57ae` after PR #106 (authoritative Media Library persistence).
...
1. **Native autosave/revisions:** manual Save/Publish/Delete are fixed, but autosave and local revision history remain browser-local. Server revision APIs already exist and should become authoritative.
2. **Backup coverage:** strict failure behavior exists, but the manifest must be re-audited after Sites, Feeds, Media, and the revision/navigation migrations are complete.

Media false-success persistence was removed in PR #106. Its remaining work is deployment/binding verification, not a known client data-integrity fallback.
