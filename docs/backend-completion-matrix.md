...
Current reconciled `main`: `1d11f447243a93487456b5c8afefc92f8b8e06c6` after PR #109 (server-authoritative native autosave and revisions). PR #108 immediately before it normalized Taxonomy and Editor Roles into the current admin shell.
...
3. **Direct RSS endpoints:** feed settings are persisted in D1, but most generated feed variants are still presented as bundle filenames instead of confirmed public XML endpoints.
4. **Production verification:** authenticated admin/API paths and current deployed asset hashes still need production confirmation.
5. **Pages:** currently a truthful route inventory, not a database-backed page CMS.
