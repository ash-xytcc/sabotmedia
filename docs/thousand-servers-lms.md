...
A learner opts in once with **Create my recovery card**. The browser stores one random identifier/key pair locally, prints that code on a terminal-style card, and reuses it for subsequent backups. Edits queue an encrypted update within a minute while the course is open and online; the learner can also update immediately. Pending/offline/conflict states are explicit. Clearing local data removes this device’s connection; restore using the same card to reconnect. Legacy snapshot codes still restore, then offer a one-time reusable-card setup.

AES-GCM uses a fresh random 96-bit IV on every update and associated data binding identifier and protocol version. A separate domain-separated, one-way SHA-256-derived write capability authorizes updates; the server stores only its hash. The AES key and plaintext never reach the server. Revision compare-and-swap prevents a stale device from overwriting a newer backup; the user must restore the newer backup or export local changes first. Lost responses are reconciled only after decrypting and comparing the remote snapshot.

The QR is generated locally at development time and links only to the public recovery screen, without a secret in the URL. Print opens a separate local document and switches to black ink on white paper. It includes the reusable code, restoration instructions, confidentiality warning, and inactivity-retention explanation. No third-party QR service is used.

Retention defaults to 365 days after each successful update; existing administrator settings remain respected. Every successful update renews retention. The code itself stays the same even if an expired blob is recreated from retained local progress; an expired backup cannot be restored without another surviving copy. Backups are limited in size and rate, with a 250 MB ciphertext admission limit. Print once, retain the card, and check that backup updates succeed before leaving the course. Anyone with the code can read and update its backup. Sabot cannot recover a lost key. JSON transfer remains available independently.
