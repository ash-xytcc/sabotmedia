...
`course_contributors` stores each project's public draft, published response, separate private text, and credential hashes. `course_contributor_revisions` stores recoverable content scoped to the project and public-edit/private permission. Passwords are separate, at least 20 characters, salted PBKDF2-SHA-256 with 100,000 iterations (the Workers Web Crypto limit). Use generated high-entropy passwords, not memorable phrases at this minimum.

Owners/admins create projects, set both passwords, or disable access through the contributor page linked from Course admin. Credentials are never generated into source or stored in plaintext. Contributor sessions use a separate cookie and domain-separated HMAC, last one hour, and are Secure/HttpOnly/SameSite=Strict. Scope, project, expiry, enabled state, and credential epoch are checked on every protected request. Password resets/disable revoke old sessions. These cookies cannot authenticate to SabotPress.

Public-edit credentials cannot unlock Private Comms or another project. Contributors submit public changes for review; editors publish. Private pages are empty authentication shells until the separate private permission is verified. Same-origin JSON checks apply to writes. Login attempts are rate limited in D1 using short-lived hashed IP/scope buckets; raw IPs are not stored in those buckets. No course content, credentials, or recovery codes are placed in analytics or audit payloads. Existing infrastructure/platform request logging remains a deployment consideration; no custom request-body logging is added.
...
Tables are created lazily using the existing BF_DB binding. SABOT_SESSION_SECRET remains required for signed contributor sessions. Before production merge, use an isolated preview D1 database to check deployed session cookies, mobile/keyboard behavior, JSON transfer, two clean browser profiles for encrypted recovery, and no-JavaScript reading. Do not test contributor credentials against a preview that shares production D1. Keep production unmerged until these acceptance checks are reviewed.

Validation at the integrated main revision 5140186: 19 focused API/cryptographic/DOM tests pass. Full suite: 439 pass, 13 fail; unchanged main has 420 pass and the same 13 failures. DOM tests are not a substitute for the outstanding real-browser acceptance checks. The combined public HTML/course prebuild and Vite build pass.
