...
function scrubSourceTracking(value) {
  if (Array.isArray(value)) return value.map(scrubSourceTracking)
  if (value && typeof value === 'object') {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, scrubSourceTracking(item)]))
  }
...
const snapshot = {
  ...
  ready: true,
  nativeContent: {
    items: scrubSourceTracking(nativeContent.items),
  },
  publicSiteConfig: {
    ...
    config: scrubSourceTracking(publicSiteConfig.config),
  },
  campaigns: {
    items: campaigns.items,
    details: scrubSourceTracking(campaignDetails),
  },
  collections: {
    items: scrubSourceTracking(collections.items),
  },
  publications: {
    items: scrubSourceTracking(publications.items),
  },
}
