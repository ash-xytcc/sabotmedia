...
const response = (html, status = 200, extra = {}) => new Response(html, {status,headers:{...headers,...extra}})
...
export async function onRequest(context) {
  ...
  try {
    ...
    if (!path || path === 'index.html') {
      ...
      return c ? response(renderCourse(c,contributors)) : response(shell('Course unavailable','<h1>Course is not currently published.</h1>'),404)
    }
    if (path === 'offline' || path === 'offline.html') {
      const c = await readCourse(db,SLUG)
      if (!c) return response(shell('Course unavailable','<h1>Course is not currently published.</h1>'),404)
      const rows = await db.prepare('SELECT id,name,published_json FROM course_contributors ORDER BY rowid').all()
      const published = (rows.results || []).filter((r)=>r.published_json).map((r)=>({id:r.id,name:r.name,item:JSON.parse(r.published_json)}))
      return response(renderOfflineEdition(c,published),200,{
        'content-disposition': `attachment; filename="become-the-thousand-servers-${String(c.contentVersion || 'current').replace(/[^a-z0-9._-]+/gi,'-')}.html"`,
      })
    }
    ...
    let published = null
    if (scope === 'edit') {
      const row = await db.prepare('SELECT published_json FROM course_contributors WHERE id=?').bind(project.id).first()
