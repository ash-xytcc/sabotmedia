...
const STATIC_PAGE_TITLES = Object.freeze({
  ...
  '/archive': 'Archive',
  '/campaigns': 'Campaigns',
  '/campaigns/autistici-inventati': 'Communications Infrastructure Is Not Terrorism',
  '/campaigns/autistici-inventati/coverage': 'A/I Campaign Coverage Archive',
...
export function resolvePageTitle(row, nativeTitles = new Map()) {
  ...
  const recorded = String(row?.title || '').replace(/\s*\|\s*Sabot Media\s*$/i, '').trim()
