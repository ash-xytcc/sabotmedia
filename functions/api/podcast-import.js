...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const decorated = decorateEpisodes(feed, existing)
    ...
    if (importChannelSettings) {
      const current = await readPodcastSettings(db)
      const now = new Date().toISOString()
      const imported = feed.parsed.podcast
      settingsResult = await writePodcastSettings(db, {
        ...current.settings,
        podcastTitle: imported.title || current.settings.podcastTitle,
        author: imported.author || current.settings.author,
        description: imported.description || current.settings.description,
        websiteUrl: imported.websiteUrl || current.settings.websiteUrl,
        defaultCoverArt: imported.imageUrl || current.settings.defaultCoverArt,
        language: imported.language || current.settings.language,
        category: imported.category || current.settings.category,
        explicit: Boolean(imported.explicit),
        ownerName: imported.ownerName || current.settings.ownerName,
        ownerEmail: imported.ownerEmail || current.settings.ownerEmail,
    ...
    await writeAuditLog(db, {
      ...
      entityType: 'podcast_feed',
      entityId: feed.sourceUrl,
      ...
      detail: {
        ...
        channelSettingsImported: Boolean(settingsResult),
      },
    })
...
function decorateEpisodes(feed, existing) {
  const sourceUrl = feed.sourceUrl
  const importedByGuid = new Map()
  const importedByEnclosure = new Map()
  for (const item of existing) {
    if (String(item?.sourceKind || '') === 'podcast-rss' && String(item?.sourceUrl || '') === sourceUrl && item?.sourceExternalId) {
      importedByGuid.set(String(item.sourceExternalId), item)
    }
...
async function importEpisodes({ db, feed, episodes, existing, syncExisting }) {
  const sourceUrl = feed.sourceUrl
  ...
  for (const item of existing) {
    if (String(item?.sourceKind || '') === 'podcast-rss' && String(item?.sourceUrl || '') === sourceUrl && item?.sourceExternalId) {
      existingByGuid.set(String(item.sourceExternalId), item)
    }
    const enclosure = publicAudioUrl(item)
  ...
  for (const episode of episodes) {
    ...
    const saved = await upsertNativeEntry(db, {
      ...
      author: episode.author || feed.parsed.podcast.author || 'Sabot Media',
      sourceType: 'rss-import',
      sourceKind: 'podcast-rss',
      sourceLabel: feed.parsed.podcast.title || 'Imported podcast RSS',
      sourceUrl,
      sourceExternalId: episode.guid,
      sourcePostId: episode.guid,
      sourceNotes: `Imported from ${feed.resolvedUrl}`,
      ...
      podcastCoverImage: episode.imageUrl || feed.parsed.podcast.imageUrl,
      featuredImage: episode.imageUrl || feed.parsed.podcast.imageUrl,
      heroImage: episode.imageUrl || feed.parsed.podcast.imageUrl,
      categories: unique(['podcast', ...(episode.categories || [])]),
      tags: unique(episode.categories || []),
