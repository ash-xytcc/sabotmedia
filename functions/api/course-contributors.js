...
export async function onRequest(context) {
  try {
    ...
    if (method === 'GET' && !url.searchParams.has('id')) {
      let items = editor ? await listContributors(db) : await listPublishedContributors(db)
      if (editor) {
    ...
    if (!row) {
      if (method === 'GET' && scope === 'edit' && !editor) return json({ ok: true, id: project, canEdit: false, editor: false, admin: false, item: null })
      return denied()
    }
    ...
    if (method === 'GET') {
      ...
      const publicVisible = !!row.published_json
      return json({
        ok: true, id: project, ...(authorized || publicVisible ? { name: row.name } : {}), canEdit: authorized, editor, admin,
