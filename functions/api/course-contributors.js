  ...
  try {
    ...
    if (body.action === 'review-decision') {
      ...
      const submission = await db
        .prepare('SELECT * FROM course_contributor_submissions WHERE id=?')
        .bind(submissionId)
        .first()
      if (!submission || submission.status !== 'pending')
        return json({ ok: false, error: 'This submission already has a decision' }, 409)
      ...
      if (decision === 'approved') {
          ...
          db
            .prepare(
              `UPDATE course_contributors
               SET published_json=?, revision=revision+1
               WHERE id=? AND revision=? AND draft_json=?`,
            )
            .bind(published, submission.project, submission.revision, submission.content_json),
          db
            .prepare(
              `UPDATE course_contributor_submissions
               SET status='approved',decision='approved',decided_at=?,reviewer_id=?,reviewer_note=?
               WHERE id=? AND status='pending'
                 AND EXISTS(SELECT 1 FROM course_contributors WHERE id=? AND revision=?)`,
            )
            .bind(now, permission.actor, reviewerNote, submissionId, submission.project, submission.revision + 1),
          db
            .prepare(
              `INSERT INTO course_contributor_revisions(id,project,version,actor_type,actor_id,scope,status,content_json,created_at)
               SELECT ?,?,?,?,?,?,?,?,?
               WHERE EXISTS(SELECT 1 FROM course_contributor_submissions WHERE id=? AND status='approved')`,
            )
            .bind(
              crypto.randomUUID(),
              submission.project,
        ...
        statements.push(
          db
            .prepare(
              `UPDATE course_contributor_submissions
               SET status=?,decision=?,decided_at=?,reviewer_id=?,reviewer_note=?
               WHERE id=? AND status='pending'`,
            )
            .bind(decision, decision, now, permission.actor, reviewerNote, submissionId),
          db
            .prepare(
              `INSERT INTO course_contributor_revisions(id,project,version,actor_type,actor_id,scope,status,content_json,created_at)
               VALUES(?,?,?,?,?,?,?,?,?)`,
      ...
      if (decision === 'approved' && decided?.status !== 'approved')
        return json(
          {
            ok: false,
            error: 'Conflict: this submission is older than the contributor’s current draft. Review the newer submission instead.',
          },
          409,
        )
    ...
    if (method === 'GET' && !url.searchParams.has('id')) {
      ...
      if (editor) {
        ...
        const pending = await db
          .prepare("SELECT * FROM course_contributor_submissions WHERE status='pending' ORDER BY submitted_at ASC")
          .all()
    ...
    if (method === 'GET') {
      ...
      if (url.searchParams.has('revisions')) {
        ...
        const rows = await db
          .prepare(
            'SELECT * FROM course_contributor_revisions WHERE project=? AND scope=? ORDER BY version DESC, created_at DESC LIMIT 100',
          )
          .bind(project, scope)
          .all()
      ...
      if (scope === 'edit' && authorized) {
        const result = await db
          .prepare('SELECT * FROM course_contributor_submissions WHERE project=? ORDER BY submitted_at DESC LIMIT 100')
          .bind(project)
          .all()
    ...
    if (scope === 'private') {
      statements.push(
        db
          .prepare('UPDATE course_contributors SET private_text=CASE WHEN revision=? THEN ? ELSE NULL END,revision=revision+1 WHERE id=?')
      ...
      statements.push(
        db
          .prepare('UPDATE course_contributors SET draft_json=CASE WHEN revision=? THEN ? ELSE NULL END,revision=revision+1 WHERE id=?')
          .bind(row.revision, nextJson, project),
      )
      if (!editor && next.status === 'review') {
        statements.push(
          db
            .prepare(
              `INSERT INTO course_contributor_submissions(id,project,revision,content_json,status)
               VALUES(?,?,?,?, 'pending')`,
            )
            .bind(crypto.randomUUID(), project, nextRevision, nextJson),
        )
      }
      if (editor && next.status === 'published') {
        statements.push(
          db.prepare('UPDATE course_contributors SET published_json=? WHERE id=?').bind(JSON.stringify(publicContribution(next)), project),
        )
      } else if (editor && next.status === 'archived') {
        statements.push(db.prepare('UPDATE course_contributors SET published_json=NULL WHERE id=?').bind(project))
      }
    }
    await db.batch(statements)
