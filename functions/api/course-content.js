...
export async function onRequest(context) {
  try {
    ...
    if (!db) return json({ ok: false, error: 'Course storage unavailable' }, 503)
    ...
    if (method === 'GET') {
      ...
      if (url.searchParams.has('revisions')) {
        if (!canPublish) return json({ ok: false, error: 'Publishing permission required' }, 403)
        const rows = await db
          .prepare('SELECT * FROM course_revisions WHERE slug=? ORDER BY created_at DESC, version DESC LIMIT 100')
          .bind(slug)
          .all()
      ...
      if (url.searchParams.get('list') === '1') {
        ...
        const items = await Promise.all(
          slugs.map(async (courseSlug) => {
            const c = await readCourse(db, courseSlug, true)
            return { slug: courseSlug, title: c.title, status: c.status, lessons: c.lessons.length }
          }),
        )
        return json({ ok: true, items })
      }
      const item = await readCourse(db, slug, canPublish && url.searchParams.get('edit') === '1')
      return item
        ? json({ ok: true, item, source: 'd1', canEdit: canPublish })
        : json({ ok: false, error: 'Course not found' }, 404)
    }

    if (!sameOrigin(context.request) || !canPublish) return json({ ok: false, error: 'Publishing permission required' }, 403)
    if (!['PUT', 'POST', 'DELETE'].includes(method)) return json({ ok: false }, 405)

    const raw = await context.request.text()
    if (raw.length > 1800000) return json({ ok: false, error: 'Course too large' }, 413)
    ...
    if (method === 'DELETE') {
      ...
      await ensureCourseTables(db)
      await db.batch([
        db.prepare('DELETE FROM course_content WHERE slug=?').bind(id(input.slug)),
        db.prepare('DELETE FROM course_publications WHERE slug=?').bind(id(input.slug)),
      ])
      return json({ ok: true })
    }
