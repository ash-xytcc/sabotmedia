  ...
  try {
    ...
    if (method === 'GET') {
      ...
      if (url.searchParams.has('revisions')) {
        ...
        const rows = await db
          .prepare(
            'SELECT * FROM course_revisions WHERE slug=? ORDER BY created_at DESC, version DESC LIMIT 100',
          )
          .bind(slug)
          .all()
      ...
      if (url.searchParams.get('list') === '1') {
        ...
        const items = await Promise.all(
          slugs.map(async (slug) => {
            const c = await readCourse(db, slug, true)
            return { slug, title: c.title, status: c.status, lessons: c.lessons.length }
          }),
        )
      ...
      return item
        ? json({ ok: true, item, source: 'd1', canEdit: permission.canEdit })
        : json({ ok: false, error: 'Course not found' }, 404)
    }
    if (!sameOrigin(context.request) || !permission.canEdit)
    ...
    if (method === 'DELETE') {
      if (!input.slug || input.slug === SLUG)
        return json({ ok: false, error: 'The seeded course cannot be deleted' }, 400)
      await ensureCourseTables(db)
      await db.batch([
    ...
    if (method === 'POST' && (await readCourse(db, id(input.slug), true)))
      return json({ ok: false, error: 'Course already exists' }, 409)
