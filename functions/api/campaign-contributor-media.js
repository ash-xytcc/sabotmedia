...
import { detectMediaSignature, mediaMimeMatchesSignature } from './_lib/mediaSignature.js'
...
export async function onRequestPost(context) {
  try {
    ...
    if (!ALLOWED.has(type)) return json({ ok: false, error: `Unsupported media type: ${declaredType || 'unknown'}. Upload a common image, audio, video, or PDF format.` }, 415)

    const bytes = new Uint8Array(await file.arrayBuffer())
    const signature = detectMediaSignature(bytes)
    if (!signature || !mediaMimeMatchesSignature(type, signature)) {
      return json({ ok: false, error: 'The file contents do not match the claimed media type.' }, 415)
    }
    ...
    await storage.bucket.put(key, bytes, { httpMetadata: { contentType: type, cacheControl: 'public, max-age=31536000, immutable' }, customMetadata: { mediaId, contributorId: contributor.id, campaignId: contributor.campaignId, source: 'campaign-contributor' } })
    const url = new URL(context.request.url); url.pathname = '/api/media/files'; url.search = ''; url.searchParams.set('key', key)
    url.searchParams.set('filename', filename)
    const mediaType = type.startsWith('audio/') ? 'audio' : type.startsWith('video/') ? 'video' : type.startsWith('image/') ? 'image' : 'document'
    try {
...
function normalizeMimeType(value, filename = '') {
  ...
  return ({ jpg: 'image/jpeg', jpeg: 'image/jpeg', png: 'image/png', webp: 'image/webp', gif: 'image/gif', heic: 'image/heic', heif: 'image/heif', webm: 'video/webm', mp4: 'video/mp4', mov: 'video/quicktime', ogv: 'video/ogg', mp3: 'audio/mpeg', m4a: 'audio/mp4', oga: 'audio/ogg', ogg: 'audio/ogg', wav: 'audio/wav', aac: 'audio/aac', pdf: 'application/pdf' })[extension] || ''
}
function extensionForMime(type, filename = '') {
  ...
  return ({ 'image/jpeg': 'jpg', 'image/png': 'png', 'image/webp': 'webp', 'image/gif': 'gif', 'image/heic': 'heic', 'image/heif': 'heif', 'audio/webm': 'webm', 'audio/ogg': 'ogg', 'audio/mpeg': 'mp3', 'audio/mp4': 'm4a', 'audio/x-m4a': 'm4a', 'audio/wav': 'wav', 'audio/x-wav': 'wav', 'audio/aac': 'aac', 'video/webm': 'webm', 'video/mp4': 'mp4', 'video/quicktime': 'mov', 'video/ogg': 'ogv', 'application/pdf': 'pdf' })[type] || 'media'
}
