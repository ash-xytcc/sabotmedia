  ...
  try {
    ...
    await writeAuditLog(db, {
      ...
      detail: {
        status: saved.status,
        slug: saved.slug,
        pieceCount: saved.pieceSlugs.length,
      },
    })
...
async function listCollections(db, options = {}) {
  ...
  const result = await db.prepare(`
    SELECT id, slug, collection_json, status, title, created_at, updated_at
    FROM collections
    ${where}
    ORDER BY title ASC
  `).all()
...
async function upsertCollection(db, item) {
  ...
  await db.prepare(`
    INSERT INTO collections (id, slug, collection_json, status, title, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
...
function rowToCollection(row) {
  let parsed = {}
  try {
    parsed = JSON.parse(row.collection_json || '{}')
  } catch {
    parsed = {}
  }
...
function normalizeCollection(input = {}) {
  ...
  return {
    ...
    featuredQuote: String(input.featuredQuote || ''),
    pieceSlugs: normalizeList(input.pieceSlugs || input.pieces),
    featuredPieceSlugs: normalizeList(input.featuredPieceSlugs || input.featuredPieces),
    relatedCollections: normalizeList(input.relatedCollections),
    relatedPieces: normalizeList(input.relatedPieces),
    timeline: normalizeRows(input.timeline, ['date', 'title', 'body']),
    downloads: normalizeRows(input.downloads, ['title', 'url', 'type']),
    gallery: normalizeRows(input.gallery, ['title', 'url', 'alt', 'caption']),
    updates: normalizeRows(input.updates, ['date', 'title', 'body', 'url']),
    externalLinks: normalizeRows(input.externalLinks, ['title', 'url']),
...
function normalizeRows(value, fields = []) {
  ...
  return arr.map((row) => {
    const normalized = { id: String(row?.id || `row-${crypto.randomUUID?.() || Math.random().toString(36).slice(2, 10)}`) }
