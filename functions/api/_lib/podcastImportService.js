import { listNativeEntries, saveRevisionSnapshot, slugify, upsertNativeEntry } from './nativePublicContent.js'
import { findPodcastShow, podcastShowSourceUrls, readPodcastShows, upsertPodcastShow } from './podcastSettings.js'
import { fetchPodcastFeed } from './podcastRssImport.js'

export const MAX_IMPORT_EPISODES = 250

export async function previewPodcastSource(db, { feedUrl, showId = '' }) {
  ...
  const episodes = decorateEpisodes(feed, existing, show)
  return {
    ...
    show,
    episodes,
    counts: summarizePreview(episodes),
  }
}

export async function importPodcastSource(db, {
  feedUrl,
  showId = '',
  selectedKeys = null,
  syncExisting = true,
  ...
  const selected = Array.isArray(selectedKeys)
    ? new Set(selectedKeys.map((value) => String(value || '')).filter(Boolean))
    : null
  const requested = decorated.filter((episode) => !selected || selected.has(episode.key))
  if (requested.length > MAX_IMPORT_EPISODES) {
    throw requestError(`import at most ${MAX_IMPORT_EPISODES} episodes at a time; select a smaller batch`, 400)
  }
  ...
  const showResult = await upsertPodcastShow(db, {
    ...
    ...(importChannelSettings ? {
      ...
      author: imported.author || show?.author || 'Sabot Media',
      description: imported.description || show?.description || '',
      websiteUrl: imported.websiteUrl || show?.websiteUrl || 'https://sabot.media',
...
async function importEpisodes({ db, feed, show, episodes, existing, syncExisting }) {
  ...
  for (const episode of episodes) {
    ...
    const saved = await upsertNativeEntry(db, {
      ...
      author: episode.author || show.author || feed.parsed.podcast.author || 'Sabot Media',
      ...
      podcastTitle: show.podcastTitle || feed.parsed.podcast.title || '',
      showTitle: show.podcastTitle || feed.parsed.podcast.title || '',
      primaryProject: show.podcastTitle || feed.parsed.podcast.title || '',
      primaryProjectSlug: show.slug || '',
      projects: unique([show.podcastTitle || feed.parsed.podcast.title || '']),
