...
export async function listPublishedContributions(db) {
  await ensureContributors(db)
  const result = await db
    .prepare(`SELECT id,name,published_json FROM course_contributors WHERE published_json IS NOT NULL AND TRIM(published_json) NOT IN ('', 'null') ORDER BY rowid`)
    .all()
  return (result.results || [])
    .filter(participatingContributor)
    .map((row) => {
      try {
        return { id: row.id, name: row.name, item: publicContribution(JSON.parse(row.published_json)) }
      } catch {
        return null
      }
    })
...
export async function listPublishedContributors(db) {
  return (await listPublishedContributions(db)).map(({ id, name }) => ({ id, name }))
}
