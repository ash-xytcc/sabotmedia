...
export function podcastFeedUrl(slug, origin = 'https://sabot.media') {
  const cleanSlug = slugifyShow(slug)
  return cleanSlug ? `${String(origin || 'https://sabot.media').replace(/\/$/, '')}/feeds/podcasts/${cleanSlug}.xml` : ''
}
...
async function migrateLegacyPodcastSettings(db) {
  ...
  const shows = orderedGroups.map((group) => {
    ...
    return normalizePodcastSettings({
      ...
      author: base.author || group.author || 'Sabot Media',
