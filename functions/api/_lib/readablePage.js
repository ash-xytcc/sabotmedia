...
export async function buildReadingPage(context, url) {
  ...
  if (family === 'feeds') {
    ...
    const shows = rows(manifest.podcastShows)
    return {title:'Feeds',body:`<h1>Follow Sabot on your own terms</h1><p>Get new articles, dispatches and podcast episodes in an app you choose. RSS lets you follow our work without a social media account or an algorithm deciding what reaches you.</p><h2>How to subscribe</h2><p>Copy a feed link into your RSS reader. For podcasts, copy a named show feed into your podcast app and choose “Add by RSS” or “Follow by URL.”</p><h2>Listen to our podcasts</h2><p>Sabot Media hosts our podcast audio and publishes these RSS feeds directly. Each show has its own feed.</p>${shows.map(show => `<p>${link(`/feeds/${show.feedPath}`,show.title || show.slug)}${show.episodeCount != null ? ` · ${e(show.episodeCount)} episodes` : ''}</p>`).join('')}<p>The named show feeds are for podcast subscriptions. The podcast format feed follows website posts across shows. The older podcasts/all.xml address follows the default show only.</p><h2>Read and follow the archive</h2>${rows(manifest.files).filter(name => !name.startsWith('podcasts/')).map(name => `<p>${link(`/feeds/${name}`,name)}</p>`).join('')}<p>Older imported pieces may not yet appear in live feeds. ${link('/archive','Browse the archive')} to find more.</p>`}
  }
