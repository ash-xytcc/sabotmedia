...
export const seed = {
  ...
  sections: sectionMap.map(([title, nums], i) => ({
    ...
    sources: [],
    contributors: [],
    completion: { manual: true, activities: [] },
    prerequisites: [],
  })),
...
const extraActivities = {
  1: ['dependency-choice'],
  2: ['dns-select', 'dns-match'],
  3: ['ssh-true-false'],
  4: ['request-sequence'],
  5: ['exposure-reflection'],
  6: ['backup-scenario', 'restore-practical'],
}
seed.lessons = seed.lessons.map((l) => ({
  ...l,
  activities: [`${l.slug}-verify`, `${l.slug}-teach`, ...(extraActivities[l.number] || [])],
}))
seed.sections[0].body =
  "The Anarchist's Guide to Losing Everyone's Email Because Dave Forgot to Patch Debian."
seed.sections[5].body =
...
const resources = (v) =>
  list(v).map((r) => ({
    title: str(r.title, 220),
    url: /^https?:\/\//i.test(r.url || '') ? str(r.url, 2000) : '',
    note: str(r.note, 1000),
  }))
const rules = (v) => ({
  manual: v?.manual !== false,
  practical: v?.practical === true,
...
export function normalizeCourse(input) {
  ...
  const result = {
    slug: id(c.slug),
    schemaVersion: 2,
    revision: Number.isSafeInteger(c.revision) ? c.revision : 0,
    ...
    const merged =
      legacy && original
        ? {
            ...original,
            ...Object.fromEntries(
              Object.entries(l).filter(([, v]) => v !== '' && (!Array.isArray(v) || v.length)),
            ),
          }
        : l
    return {
      ...
      time: str(merged.time, 80),
      status: status(legacy ? 'published' : merged.status),
      ...Object.fromEntries(['learn', 'do', 'test', 'teach'].map((k) => [k, str(merged[k])])),
      objectives: strings(merged.objectives),
      checks: strings(merged.checks),
      resources: resources(merged.resources),
      activities: strings(merged.activities),
      prerequisites: strings(merged.prerequisites),
      enforcePrerequisites: merged.enforcePrerequisites === true,
      completion: rules(merged.completion),
    }
  })
  result.sections = list(c.sections, 60).map((s) => ({
    id: str(s.id, 120),
    title: str(s.title, 220),
    body: str(s.body),
    status: status(s.status),
    lessonSlugs: strings(s.lessonSlugs),
    activities: strings(s.activities),
    sources: resources(s.sources),
    contributors: strings(s.contributors),
    prerequisites: strings(s.prerequisites),
    completion: rules(s.completion),
  }))
  if (result.slug === SLUG) {
    if (
      result.sections.length !== 13 ||
      result.sections.some(
        (s, i) =>
          s.id !== seed.sections[i].id ||
  ...
  result.activities = list(c.activities, 200).map((a) => {
    ...
    return {
      ...
      title: str(a.title, 220),
      prompt: str(a.prompt, 5000),
      status: status(a.status),
      options,
      answer: strings(a.answer),
      pairs: list(a.pairs, 30).map((p) => ({ left: str(p.left, 1000), right: str(p.right, 1000) })),
      feedback: str(a.feedback, 5000),
      sources: resources(a.sources),
    }
  })
  ...
  result.documents = list(c.documents, 30).map((d) => ({
    ...
    sources: resources(d.sources),
  }))
  result.testing = list(c.testing, 300).map((t) =>
    Object.fromEntries(
      [
        'id',
        'exercise',
...
export function publicCourse(c) {
  ...
  for (const key of ['lessons', 'sections', 'activities', 'documents'])
    out[key] = out[key].filter((x) => x.status === 'published')
  out.testing = []
  return out
}
export { legacySeed }
