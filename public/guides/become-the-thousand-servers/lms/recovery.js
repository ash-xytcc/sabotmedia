...
export async function encryptProgress(state, existingCode = null) {
  ...
  const bytes = new Uint8Array(
    await crypto.subtle.encrypt(
      { name: 'AES-GCM', iv, additionalData: new TextEncoder().encode(`sabot-course:1:${id}`) },
      key,
...
export function parseCode(code) {
  ...
  if (
    parts.length !== 3 ||
    parts[0] !== 'sabot1' ||
    !parts.slice(1).every((p) => /^[A-Za-z0-9_-]{43}$/.test(p))
  )
  ...
  const plain = await crypto.subtle.decrypt(
    {
      name: 'AES-GCM',
      iv: decode(blob.iv),
      additionalData: new TextEncoder().encode(`sabot-course:1:${p.id}`),
    },
    key,
