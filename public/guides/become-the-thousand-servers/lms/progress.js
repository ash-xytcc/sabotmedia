...
export const blank=()=>({course:SLUG,version:2,contentVersion:seed.contentVersion,completedLessons:[],startedLessons:[],completedSections:[],startedSections:[],viewed:[],practical:[],lastLesson:'',lastUnit:'',scrollByLesson:{},bookmarks:[],notes:{},activities:{}})
...
export function activityPassed(c,s,id) {const a=c.activities.find(a=>a.id===id);return !!a&&!!s.activities[id]?.some(t=>t.version===a.version&&t.passed)}
export function complete(c,s,unit) {
 const key=unit.slug||unit.id,r=unit.completion||{manual:true}
 const manual=(unit.slug?s.completedLessons:s.completedSections).includes(key)
 return (!r.manual||manual)&&(!r.viewed||s.viewed.includes(key))&&(!r.practical||s.practical.includes(key))&&(r.activities||[]).every(id=>activityPassed(c,s,id))
}
