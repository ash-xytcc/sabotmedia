(() => {
  const FALLBACK = [
    ...
    { id:'pr-ai-ofac-origin', publicTitle:'The earliest record putting A/I into the sanctions pipeline', whyItMatters:'This is the central missing bureaucratic handoff: the first record that identifies A/I as a proposed sanctions target, the originating office, and the basis for opening the targeting file.', recordsSought:'Records sufficient to identify the earliest referral, recommendation, targeting memorandum, case initiation, originating office, and evidentiary basis that proposed Autistici/Inventati for designation under Executive Order 13224.', agencyName:'U.S. Department of the Treasury', agencyComponentName:'Component not yet verified', status:'Drafting', dateFiled:'', trackingNumber:'', lastUpdated:'2026-09-04', requestText:'', requestTextPublic:false, documents:[] },
    { id:'pr-ai-dhs-fbi-infrastructure', publicTitle:'Federal infrastructure research and onward referrals involving NoBlogs/A/I', whyItMatters:'Federal investigators encountered NoBlogs-linked infrastructure before the final designation. These records could show whether that investigative knowledge was later referred into the sanctions process.', recordsSought:'Records identifying NoBlogs, Autistici/Inventati, Abolition Media, or Rose City Counter-Info as infrastructure in federal investigations, and any onward referral to State, Treasury/OFAC, DOJ, or other components.', agencyName:'Federal component to be verified', agencyComponentName:'', status:'Researching', dateFiled:'', trackingNumber:'', lastUpdated:'2026-09-04', requestText:'', requestTextPublic:false, documents:[] }
  ]
  ...
  fetch('/api/public-records?investigation=autistici-inventati', { headers: { accept:'application/json' } })
    ...
    .then((data) => render(data.requests || [], data.summary || summarize(data.requests || []), false))
