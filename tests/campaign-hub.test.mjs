...
test('campaign coverage and official statements refresh automatically with strict server-side sources', () => {
  assert.match(intelligence, /keepitfree\.ai\/it\/index\.xml/)
  assert.match(socialServer, /mastodon\.bida\.im['"], acct: ['"]cavallette/)
  assert.match(socialServer, /api\/v1\/accounts\/lookup/)
...
test('campaign core renders before slow live sources and secondary post loading', () => {
  const campaignApi = fs.readFileSync(new URL('../functions/api/campaigns.js', import.meta.url), 'utf8')
  const intelligenceEndpoint = fs.readFileSync(new URL('../functions/api/campaign-intelligence.js', import.meta.url), 'utf8')
  assert.match(campaignApi, /includeSocial: false, includeIntelligence: false/)
  assert.match(page, /fetch\('\/api\/campaign-social\?slug='/)
  assert.match(page, /fetch\('\/api\/campaign-intelligence'/)
  assert.ok(page.indexOf('setCampaign(loadedCampaign)') < page.indexOf('loadPublishedNativePieces()'))
  assert.match(intelligenceEndpoint, /loadLiveAiIntelligence/)
})
