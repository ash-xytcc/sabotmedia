...
const pagesRoutes = JSON.parse(fs.readFileSync(new URL('../public/_routes.json', import.meta.url), 'utf8'))
const publicSignatureRoute = fs.readFileSync(new URL('../functions/campaign-signatures-public.js', import.meta.url), 'utf8')
...
test('public signature write route is included in Pages Functions routing', () => {
  assert.ok(pagesRoutes.include.includes('/campaign-signatures-public'))
  assert.match(publicSignatureRoute, /onRequestPost/)
  assert.match(publicSignatureRoute, /handlePost/)
})
