...
async function unlock(f,id='puscii'){await req(f,'/api/course-contributors',{action:'create',id,name:'Test Contributor'},f.cookie);await req(f,'/api/course-contributors',{action:'credentials',id,editPassword:'phase-two-contributor-password-123'},f.cookie);const login=await req(f,'/api/course-contributors',{action:'login',id,password:'phase-two-contributor-password-123'});return login.r.headers.get('set-cookie').split(';')[0]}
