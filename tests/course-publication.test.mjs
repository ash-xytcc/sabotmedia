...
test('web edition is expanded, sober, course-first, and hides unpublished outreach', () => {
  ...
  assert.match(html, /Software options worth comparing/)
  assert.match(html, /writefreely\.org\/docs/)
  assert.match(html, /developer\.wordpress\.org\/advanced-administration\/wordpress\/import/)
  assert.match(html, /ghost\.org\/help\/imports/)
  assert.match(html, /learn\.getgrav\.org\/2\/migration\/migrating-to-grav/)
  assert.match(html, /A WXR-only NoBlogs archive may therefore need a different conversion path/)
  assert.match(html, /Free to self-host\. Ghost\(Pro\) managed hosting starts at \$18\/month billed yearly\./)
  assert.match(html, /Write\.as starts at \$6\/month when paid yearly/)
  assert.match(html, /WordPress\.com has a free hosted tier/)
  assert.match(html, /Yes — GPL-3\.0/)
  assert.match(html, /Yes — AGPL-3\.0/)
  assert.match(html, /Yes — GPL-2\.0-or-later/)
  assert.match(html, /Yes — MIT/)
  assert.match(html, /ghost\.org\/pricing/)
  assert.match(html, /wordpress\.com\/pricing/)
  assert.doesNotMatch(html, /\[\[SOFTWARE_OPTIONS\]\]/)
...
test('supplemental recovery canonical source contains the reviewed technical copy', () => {
  ...
  assert.match(byId('blog-destination').body, /\[\[SOFTWARE_OPTIONS\]\]/)
